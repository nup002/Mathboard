Every symbol that can be typed with the Mathpad is listed here. 
This page is still a work in progress, but every symbol is present.

## Lowercase Greek Letters
<div class="symbols-grid">
  <div class="symbol-card">
    <div class="symbol">α</div>
    <div class="name">Alpha</div>
    <div class="details">
      <div class="key-combo"><keycombo>[α]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">β</div>
    <div class="name">Beta</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R]+[α]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">γ</div>
    <div class="name">Gamma</div>
    <div class="details">
      <div class="key-combo"><keycombo>[γ]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">δ</div>
    <div class="name">Delta</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R]+[γ]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">ε</div>
    <div class="name">Epsilon</div>
    <div class="details">
      <div class="key-combo"><keycombo>[ε]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">ζ</div>
    <div class="name">Zeta</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R]+[ε]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">η</div>
    <div class="name">Eta</div>
    <div class="details">
      <div class="key-combo"><keycombo>[η]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">θ</div>
    <div class="name">Theta</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R]+[η]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">ι</div>
    <div class="name">Iota</div>
    <div class="details">
      <div class="key-combo"><keycombo>[ι]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">κ</div>
    <div class="name">Kappa</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R]+[κ]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">λ</div>
    <div class="name">Lambda</div>
    <div class="details">
      <div class="key-combo"><keycombo>[λ]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">μ</div>
    <div class="name">Mu</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R]+[μ]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">ν</div>
    <div class="name">Nu</div>
    <div class="details">
      <div class="key-combo"><keycombo>[ν]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">ξ</div>
    <div class="name">Xi</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R]+[ξ]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">π</div>
    <div class="name">Pi</div>
    <div class="details">
      <div class="key-combo"><keycombo>[π]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">ρ</div>
    <div class="name">Rho</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R]+[ρ]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">σ</div>
    <div class="name">Sigma</div>
    <div class="details">
      <div class="key-combo"><keycombo>[σ]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">τ</div>
    <div class="name">Tau</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R]+[τ]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">υ</div>
    <div class="name">Upsilon</div>
    <div class="details">
      <div class="key-combo"><keycombo>[υ]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">φ</div>
    <div class="name">Phi</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R]+[φ]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">χ</div>
    <div class="name">Chi</div>
    <div class="details">
      <div class="key-combo"><keycombo>[χ]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">ψ</div>
    <div class="name">Psi</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R]+[ψ]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">ω</div>
    <div class="name">Omega</div>
    <div class="details">
      <div class="key-combo"><keycombo>[ω]</keycombo></div>
    </div>
  </div>
</div>

## Uppercase Greek Letters
<div class="symbols-grid">
  <div class="symbol-card">
    <div class="symbol">Γ</div>
    <div class="name">Gamma (uppercase)</div>
    <div class="details">
      <div class="key-combo"><keycombo>[γ][x2]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">Δ</div>
    <div class="name">Delta (uppercase)</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R]+[δ][x2]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">Θ</div>
    <div class="name">Theta (uppercase)</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R]+[θ][x2]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">Λ</div>
    <div class="name">Lambda (uppercase)</div>
    <div class="details">
      <div class="key-combo"><keycombo>[λ][x2]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">Ξ</div>
    <div class="name">Xi (uppercase)</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R]+[ξ][x2]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">Π</div>
    <div class="name">Pi (uppercase)</div>
    <div class="details">
      <div class="key-combo"><keycombo>[π][x2]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">Σ</div>
    <div class="name">Sigma (uppercase)</div>
    <div class="details">
      <div class="key-combo"><keycombo>[σ][x2]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">Φ</div>
    <div class="name">Phi (uppercase)</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R]+[φ][x2]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">Ψ</div>
    <div class="name">Psi (uppercase)</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R]+[ψ][x2]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">Ω</div>
    <div class="name">Omega (uppercase)</div>
    <div class="details">
      <div class="key-combo"><keycombo>[ω][x2]</keycombo></div>
    </div>
  </div>
</div>

## Relationships
<div class="symbols-grid">
  <div class="symbol-card">
    <div class="symbol">≠</div>
    <div class="name">Not equal</div>
    <div class="details">
      <div class="key-combo"><keycombo>[C]+[α]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">≈</div>
    <div class="name">Approximately equal</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[α]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∝</div>
    <div class="name">Proportional to</div>
    <div class="details">
      <div class="key-combo"><keycombo>[C]+[β]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">≃</div>
    <div class="name">Asymptotically equal</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[β]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">≅</div>
    <div class="name">Approximately equal</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[β][x2]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">≡</div>
    <div class="name">Identical to</div>
    <div class="details">
      <div class="key-combo"><keycombo>[C]+[ε]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">≣</div>
    <div class="name">Strictly equivalent to</div>
    <div class="details">
      <div class="key-combo"><keycombo>[C]+[ε][x2]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">≔</div>
    <div class="name">Definition</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[ε]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">≝</div>
    <div class="name">Definition (alternative)</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[ε][x2]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">≥</div>
    <div class="name">Greater or equal</div>
    <div class="details">
      <div class="key-combo"><keycombo>[C]+[η]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">≤</div>
    <div class="name">Less or equal</div>
    <div class="details">
      <div class="key-combo"><keycombo>[C]+[η][x2]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">≫</div>
    <div class="name">Much greater</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[η]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">≪</div>
    <div class="name">Much less</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[η][x2]</keycombo></div>
    </div>
  </div>
</div>

## Calculus
<div class="symbols-grid">
  <div class="symbol-card">
    <div class="symbol">∑</div>
    <div class="name">Summation</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∑]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∏</div>
    <div class="name">Product</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∏]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∫</div>
    <div class="name">Integral</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∫]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∬</div>
    <div class="name">Double integral</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∬]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∭</div>
    <div class="name">Triple integral</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∭]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∮</div>
    <div class="name">Contour integral</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∮]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∯</div>
    <div class="name">Surface integral</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∯]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∰</div>
    <div class="name">Volume integral</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∰]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">√</div>
    <div class="name">Square root</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[√]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∛</div>
    <div class="name">Cube root</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∛]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∜</div>
    <div class="name">Quadratic root</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∜]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∂</div>
    <div class="name">Partial differential</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∂]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∇</div>
    <div class="name">Nabla</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∇]</keycombo></div>
    </div>
  </div>
</div>

## Logic
<div class="symbols-grid">
  <div class="symbol-card">
    <div class="symbol">∧</div>
    <div class="name">Logical And</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∧]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∨</div>
    <div class="name">Logical Or</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∨]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">¬</div>
    <div class="name">Not</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[¬]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∃</div>
    <div class="name">There Exists</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∃]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∄</div>
    <div class="name">There Does Not Exist</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∄]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∀</div>
    <div class="name">For all</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∀]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">⊤</div>
    <div class="name">Down tack</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[⊤]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">⊥</div>
    <div class="name">Up Tack</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[⊥]</keycombo></div>
    </div>
  </div>
</div>

## Arrows
<div class="symbols-grid">
  <div class="symbol-card">
    <div class="symbol">→</div>
    <div class="name">Right arrow</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[→]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">↔</div>
    <div class="name">Left Right arrow</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[↔]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">←</div>
    <div class="name">Left arrow</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[←]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">⇒</div>
    <div class="name">Right double arrow</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[⇒]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">⇔</div>
    <div class="name">Left Right double arrow</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[⇔]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">⇐</div>
    <div class="name">Left double arrow</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[⇐]</keycombo></div>
    </div>
  </div>
</div>

## Diacritics
<div class="symbols-grid">
  <div class="symbol-card">
    <div class="symbol">^</div>
    <div class="name">Hat</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[^]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">ˇ</div>
    <div class="name">Check</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[ˇ]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">~</div>
    <div class="name">Tilde</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[~]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">¯</div>
    <div class="name">Overbar</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[¯]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">⃗</div>
    <div class="name">Arrow</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[⃗]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">˙</div>
    <div class="name">Dot</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[˙]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">¨</div>
    <div class="name">Double dot</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[¨]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">⃛</div>
    <div class="name">Triple dot</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[⃛]</keycombo></div>
    </div>
  </div>
</div>

## Subscript / Superscript
<div class="symbols-grid">
  <div class="symbol-card">
    <div class="symbol">₀</div>
    <div class="name">Subscript 0</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[₀]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">₁</div>
    <div class="name">Subscript 1</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[₁]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">₂</div>
    <div class="name">Subscript 2</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[₂]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">⁰</div>
    <div class="name">Superscript 0</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[⁰]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">¹</div>
    <div class="name">Superscript 1</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[¹]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">²</div>
    <div class="name">Superscript 2</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[²]</keycombo></div>
    </div>
  </div>
</div>

## Set Theory
<div class="symbols-grid">
  <div class="symbol-card">
    <div class="symbol">∈</div>
    <div class="name">Element of</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∈]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∋</div>
    <div class="name">Contains as member</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∋]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∉</div>
    <div class="name">Not an element of</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∉]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">⊂</div>
    <div class="name">Proper Subset</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[⊂]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">⊄</div>
    <div class="name">Not proper subset</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[⊄]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">⊆</div>
    <div class="name">Subset</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[⊆]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">⊈</div>
    <div class="name">Not subset</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[⊈]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∪</div>
    <div class="name">Union</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∪]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∩</div>
    <div class="name">Intersection</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∩]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">⊕</div>
    <div class="name">Symmetric Difference</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[⊕]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">⊔</div>
    <div class="name">Disjoint Union</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[⊔]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∅</div>
    <div class="name">Empty set</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∅]</keycombo></div>
    </div>
  </div>
</div>

## Other Symbols
<div class="symbols-grid">
  <div class="symbol-card">
    <div class="symbol">×</div>
    <div class="name">Cross product</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[×]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">⋅</div>
    <div class="name">Dot product</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[⋅]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">±</div>
    <div class="name">Plus minus</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[±]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∓</div>
    <div class="name">Minus plus</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∓]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">°</div>
    <div class="name">Degree</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[°]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">∞</div>
    <div class="name">Infinity</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[∞]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">⁄</div>
    <div class="name">Fraction</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[⁄]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">⊗</div>
    <div class="name">Tensor product</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[⊗]</keycombo></div>
    </div>
  </div>
</div>

## Number Systems
<div class="symbols-grid">
  <div class="symbol-card">
    <div class="symbol">ℕ</div>
    <div class="name">Natural numbers</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[ℕ]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">ℤ</div>
    <div class="name">Integer numbers</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[ℤ]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">ℚ</div>
    <div class="name">Rational numbers</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[ℚ]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">ℝ</div>
    <div class="name">Real numbers</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[ℝ]</keycombo></div>
    </div>
  </div>
  <div class="symbol-card">
    <div class="symbol">ℂ</div>
    <div class="name">Complex numbers</div>
    <div class="details">
      <div class="key-combo"><keycombo>[R][C]+[ℂ]</keycombo></div>
    </div>
  </div>
</div>
